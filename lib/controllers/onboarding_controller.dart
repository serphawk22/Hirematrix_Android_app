...
class OnboardingController extends GetxController {
  ...
  Future<void> selectExperienceDate(
    ...
    try {
      ...
      if (result != null && result.files.single.path != null) {
        ...
        if (response.statusCode == 200 && resData['status'] == 'success') {
          ...
          Get.snackbar(
            'Success',
            'Resume parsed & populated successfully!',
      ...
      Get.snackbar(
        'Error',
        'Error uploading/parsing resume: $e',
        snackPosition: SnackPosition.BOTTOM,
        backgroundColor: Colors.red,
        colorText: Colors.white,
      );
    ...
    if (currentStep.value == 0) {
      ...
      for (var edu in educations) {
        if (edu['degree']!.text.isEmpty ||
          ...
          Get.snackbar(
            'Required Fields',
            'Please complete all education details.',
            snackPosition: SnackPosition.BOTTOM,
            backgroundColor: Colors.orange,
            colorText: Colors.white,
          );
  ...
  Future<bool> saveEducation() async {
    isLoading.value = true;
    try {
      final educationPayload = educations
          .map(
            (edu) => {
  ...
  Future<bool> saveExperience() async {
    isLoading.value = true;
    try {
      final experiencePayload = isFresher.value
          ? []
          : experiences
                .map(
                  (exp) => {
  ...
  Future<bool> completeOnboarding() async {
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({'user_id': userId, 'step': 'review'}),
      );
